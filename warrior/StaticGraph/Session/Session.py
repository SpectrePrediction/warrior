...
class Session(object):
    ...
    def save(self, checkpoint_path, is_cover=False, write_step=None):
        ...
        for trainable_op in self.graph.trainable_variables:
            ...
            if trainable_op.tensor.get_shape():
                ...
                Error("存在变量无法获取shape！{}".format(trainable_op), is_raise=True, exc_info=SaveError)

            save_dict[str(trainable_op)] = axis_list
    ...
    def restore(self, checkpoint_path):
        ...
        try:
            ...
            Error("模型损坏！无法读取有效内容", is_raise=True, exc_info=err)
        except FileNotFoundError as err:
            Error("提供路径下没有模型！", is_raise=True, exc_info=err)
        for key, value in op_dict.items():
            ...
            for trainable_op in self.graph.trainable_variables:
                if trainable_op.tensor.name.split(' ')[0] == name:
                    if trainable_op.tensor.dtype.split("'")[1] == dtype:
